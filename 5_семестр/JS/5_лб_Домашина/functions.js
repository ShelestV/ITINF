function sortByArray(data, sortArray) {
...
function sort(data, byThat) {
    if (byThat === "numberOfReviews") {
        let numberOfReviewsData = [];
        data.forEach((product) => {
            numberOfReviewsData.push(parseInt(product.ratingRevievs));
        });

        return this.sortByArray(data, numberOfReviewsData);
    } else if (byThat === "price") {
        let prices = [];
        data.forEach((product) => {
            prices.push(parseInt((product.price.newUan === undefined) ? product.price : product.price.newUan));
        });

        return this.sortByArray(data, prices);
    }

    return data;
}

function sortByNumberOfReviews() {

    this.showData(this.sort(this.getData(), "numberOfReviews"));
}

function sortByPrice() {

    this.showData(this.sort(this.getData(), "price"));
}
...
function showData(data) {
    ...
    show.innerText = "";
    data.forEach((product) => {
        let price = (product.price.newUan === undefined) ? product.price : product.price.newUan;
        show.innerText += product.name + "\n" + price + "\n" + product.ratingRevievs + "\n\n";
    });
}
