...
{
    public static async Task<IEnumerable<TInput[]>> MultiCartesianAsync<TInput>(this IEnumerable<IEnumerable<TInput>> input)
    {
        return await Task.Run(() => input.MultiCartesian(x => x));
    }
